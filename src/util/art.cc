 ...
 * the key and node.
 */
static int check_prefix(const art_node *n, const uint8_t *key, int key_len, int depth) {
 ...
 * @return 0 on success.
 */
static int leaf_matches(const art_leaf *n, const uint8_t *key, int key_len, int depth) {
    (void)depth;
 ...
 */
static int active_leaf_prefix_matches(const art_leaf *n, const uint8_t *key, int key_len, int depth,
                                      void **ret_ptr) {
    (void)depth;
    if(n->value){ // if regular key
        if(!memcmp(n->key, key, min(key_len, n->key_len)) && (n->key_len == (uint32_t)key_len)){
            // if match exactly
            *ret_ptr = n->value;
            return 1;
        }
 ...
 */
void* art_search(const art_tree *t, const uint8_t *key, int key_len) {
    art_node **child;
    art_node *n = t->root;
...
static art_leaf* make_leaf(const uint8_t *key, int key_len, void *value) {
 ...
 */
static int prefix_mismatch(const art_node *n, const uint8_t *key, int key_len, int depth) {
...
static void* recursive_insert(art_node *n, art_node **ref, const uint8_t *key, int key_len, void *value, int depth, int *old, art_tree* tree, int active_index) {
 ...
 */
static int leaf_disable_prefix(art_tree* tree, art_leaf *n, art_node **ref, const uint8_t *key, int key_len, int depth) {
    (void) depth;
    int max_cmp = min(n->key_len, key_len) - depth;
    int idx;
    for (idx = 0; idx < max_cmp; idx++) {
    ...
    if (idx == key_len - depth) {
        ...
        memcpy(new_node->n.partial, key + depth, min(MAX_PREFIX_LEN, idx));

        // Add the leafs to the new node4
        *ref = (art_node *) new_node;

        add_child4(new_node, ref, n->key[depth + idx], SET_LEAF(n));
        add_child4(new_node, ref, l2->key[depth + idx], SET_LEAF(l2));
    }
 ...
 */
void* art_insert(art_tree *t, const uint8_t *key, int key_len, void *value) {
    int old_val = 0;
...
static void remove_child(art_node *n, art_node **ref, uint8_t c, art_node **l, int depth) {
    switch (n->type) {
        case NODE4:
...
static art_leaf* recursive_delete(art_node *n, art_node **ref, const uint8_t *key, int key_len, int depth) {
    // Search terminated
    if (!n) return NULL;
 ...
 * the value pointer is returned.
 */
void* art_delete(art_tree *t, const uint8_t *key, int key_len) {
 ...
 * @return 1 on success, 0 on not found, -1 on already disabled
 */
int art_disable(art_tree *t, const uint8_t *key, int key_len) {
    art_node *n = t->root;
    art_node **ref = &(t->root);
    int depth = 0;
    // Search terminated
    ...
    while (n) {
        ...
        if (n->partial_len) {
            ...
            int prefix_len = prefix_mismatch(n, key, key_len, depth);
            ...
            if (prefix_len < key_len - depth && ((uint32_t) prefix_len < n->partial_len)) {
                // Create a new node
                art_node4 *new_node = (art_node4 *) alloc_node(NODE4);
                // *ref = (art_node*)new_node;
                *ref = (art_node *) new_node;
 ...
 * @return -1 on disabled, 0 on not found, 1 on found
 */
int art_lookup(const art_tree* t, const uint8_t* key, int key_len, void** ret_ptr){
    art_node **child;
    art_node *n = t->root;
    int prefix_len, depth = 0;
    while (n) {
 ...
 * @return 0 on success.
 */
static int leaf_prefix_matches(const art_leaf *n, const uint8_t *prefix, int prefix_len) {
 ...
 * @return 0 on success, or the return of the callback.
 */
int art_iter_prefix(art_tree *t, const uint8_t *key, int key_len, art_callback cb, void *data) {
    art_node **child;
    art_node *n = t->root;
    int prefix_len, depth = 0;
    while (n) {
 ...
 * @return 0 on success, or the return of the callback.
 */
void art_iter_active_prefix(art_tree *t, const uint8_t *key, int key_len, art_prefix_callback cb, void *data) {
    art_node **child;
    art_node *n = t->root;
    int prefix_len, depth = 0;
    while (n) {
...
int art_delete_prefix(art_tree *t, const uint8_t *key, int key_len, int depth) {
    art_node* n = t->root;
    art_node **ref = &(t->root);
    // Search terminated
    if (!n) return 0;
    ...
    while(n){
        ...
        if ((*child)->partial_len) {
            // if prefix needs to be resolved at a non leaf child with partial prefix
            int prefix_len = prefix_mismatch(*child, key, key_len, depth);
